...
export default function DashboardPage() {
    ...
    try {
      ...
      if (!profileData) {
        const { data: newProfile } = await supabase
          .from('profiles').insert({ user_id: user.id }).select().single()
      ...
      if (profileData) {
        const { data: linksData } = await supabase
          .from('links').select('*').eq('profile_id', profileData.id)
      ...
      if (profileData?.role === 'admin') {
        ...
        const { data: purchaseData } = await supabase
          .from('purchases').select('id').eq('user_id', user.id)
          .eq('item_type', 'logo_remove').eq('status', 'paid').single()
  ...
  const handleAddLink = async () => {
    ...
    const { data, error } = await supabase.from('links').insert({
      ...
      link_type: 'text', link_size: 'small', sort_order: textLinks.length, is_active: true,
    }).select().single()
  ...
  const handleUpdateLink = async () => {
    ...
    if (!error) {
      ...
      setEditingLink(null); setShowLinkForm(false)
    }
    setSaving(false)
  }
  ...
  const handleTextLinkDragEnd = async (event: DragEndEvent) => {
    ...
    const updatedLinks = links.map((l) => {
      if (l.link_type !== 'text') return l
      const newOrder = reordered.findIndex((r) => r.id === l.id)
      return { ...l, sort_order: newOrder }
    })
    setLinks(updatedLinks)

    await Promise.all(
      reordered.map((l, idx) =>
        supabase.from('links').update({ sort_order: idx }).eq('id', l.id)
      )
    )
  }
  ...
  const handleSaveImageLink = async () => {
    ...
    if (editingImageLink.id) {
      const { error } = await supabase.from('links').update({
        url: editingImageLink.url, image_url: imageUrl,
        link_size: editingImageLink.link_size, updated_at: new Date().toISOString(),
      }).eq('id', editingImageLink.id)
      if (!error) {
        setLinks(links.map((l) => l.id === editingImageLink.id
          ? { ...l, url: editingImageLink.url, image_url: imageUrl!, link_size: editingImageLink.link_size }
          : l
        ))
      }
    } else {
      const { data, error } = await supabase.from('links').insert({
        ...
        link_type: 'image', image_url: imageUrl, link_size: editingImageLink.link_size,
        sort_order: imageLinks.length, is_active: true,
      }).select().single()
  ...
  // ── Image link drag-and-drop ──

  const handleImageLinkDragEnd = async (event: DragEndEvent) => {
    const { active, over } = event
    if (!over || active.id === over.id) return

    const imageLinks = links.filter((l) => l.link_type === 'image')
    const oldIdx = imageLinks.findIndex((l) => l.id === active.id)
    const newIdx = imageLinks.findIndex((l) => l.id === over.id)
    const reordered = arrayMove(imageLinks, oldIdx, newIdx)

    const updatedLinks = links.map((l) => {
      if (l.link_type !== 'image') return l
      const newOrder = reordered.findIndex((r) => r.id === l.id)
      return { ...l, sort_order: newOrder }
    })
    setLinks(updatedLinks)
  ...
  const handleSaveGalleryPhoto = async () => {
    ...
    if (editingGalleryPhoto.file) {
      ...
      const { error: uploadError } = await supabase.storage.from('avatars')
        .upload(path, editingGalleryPhoto.file, { upsert: false })
    ...
    if (editingGalleryPhoto.id) {
      ...
      }).eq('id', editingGalleryPhoto.id)
      if (!error) {
        setLinks(links.map((l) => l.id === editingGalleryPhoto.id ? { ...l, image_url: imageUrl! } : l))
      }
    } else {
      const { data, error } = await supabase.from('links').insert({
        profile_id: profile.id, title: '', url: '',
        link_type: 'gallery', image_url: imageUrl, link_size: 'small',
  ...
  const handleGalleryMove = async (linkId: string, direction: 'up' | 'down') => {
    ...
    const updatedLinks = links.map((l) => {
      if (l.link_type !== 'gallery') return l
      const newOrder = reordered.findIndex((r) => r.id === l.id)
      return { ...l, sort_order: newOrder }
    })
    setLinks(updatedLinks)
    ...
    await Promise.all(
      reordered.map((l, i) => supabase.from('links').update({ sort_order: i }).eq('id', l.id))
    )
  }
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <header className="sticky top-0 z-40 bg-[#0a0a0a]/90 backdrop-blur-md border-b border-white/5 px-5 py-4">
        <div className="max-w-lg mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            {profile?.role === 'admin' && (
              <Link href="/admin" className="text-[13px] text-[#d4af37] hover:text-[#e8cc6a] transition-colors font-semibold">
                Admin
              </Link>
            )}
            {profile && (
              <Link href={`/u/${profile.user_id}`} target="_blank" className="text-[13px] text-gray-500 hover:text-white transition-colors">
                公開ページ ↗
              </Link>
            )}
            <button onClick={handleLogout} className="text-[13px] text-gray-600 hover:text-gray-300 transition-colors">
              ログアウト
            </button>
          </div>
        </div>
      ...
      <main className="max-w-lg mx-auto px-5 py-8 space-y-5 pb-safe">
        ...
        {profile && (
          <section
            className="rounded-3xl border border-[#d4af37]/15 p-5"
            style={{ background: 'linear-gradient(160deg, #131008, #0d0d0d)' }}
          >
            <div className="flex items-center gap-2 mb-4">
              <div className="w-1.5 h-1.5 rounded-full bg-[#d4af37]" />
              <p className="text-[11px] font-bold text-[#d4af37] tracking-[0.12em] uppercase">
                あなたの公開ページ
              </p>
            </div>
            <div className="flex items-center gap-3 mb-3">
              ...
              <button
                ...
                className={`flex-none px-4 py-3 rounded-xl text-[12px] font-bold transition-all active:scale-[0.97] ${
                  urlCopied ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-[#d4af37] text-black hover:bg-[#e8cc6a]'
                }`}
              >
                {urlCopied ? '済み ✓' : 'コピー'}
              </button>
            </div>
            <a href={`/u/${profile.user_id}`} target="_blank" rel="noopener noreferrer" className="text-[12px] text-gray-500 hover:text-gray-300 transition-colors">
              公開ページを開く ↗
            </a>
          </section>
        )}
            ...
            <div className="flex gap-2.5 pt-1">
              ...
              <button onClick={handleSaveProfile} disabled={saving}
                className={`flex-1 py-3 text-[13px] font-bold rounded-xl transition-all active:scale-[0.98] disabled:opacity-50 ${
                  saved ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-white text-black hover:bg-gray-100'
                }`}>
        ...
        <section className="bg-[#111] rounded-3xl border border-white/8 p-5">
          ...
          {textLinks.length === 0 ? (
            <div className="py-8 text-center">
              ...
              <button onClick={() => { setEditingLink({ id: null, title: '', url: '' }); setShowLinkForm(true) }}
                className="text-[13px] text-[#d4af37] hover:text-[#e8cc6a] transition-colors">
                + 最初のリンクを追加する
              </button>
            </div>
          ) : (
            <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleTextLinkDragEnd}>
        ...
        <section className="bg-[#111] rounded-3xl border border-white/8 p-5">
          <div className="flex items-center justify-between mb-5">
            <p className="text-[11px] font-bold text-gray-500 tracking-[0.12em] uppercase">画像リンク</p>
            <button
              onClick={() => {
                setEditingImageLink({ id: null, url: '', image_url: null, previewSrc: null, file: null, link_size: 'small' })
          ...
          {showImageForm && editingImageLink && (
            <div className="mb-4 p-4 bg-white/5 border border-white/8 rounded-2xl space-y-3">
              {/* Size selector */}
              <div>
                <p className="text-[11px] text-gray-500 mb-2">サイズ</p>
                <div className="grid grid-cols-2 gap-2">
                  {(['small', 'large'] as const).map((size) => (
                    <button
                      key={size}
                      onClick={() => setEditingImageLink({ ...editingImageLink, link_size: size })}
                      className={`py-2.5 rounded-xl text-[12px] font-semibold border transition-all ${
                        editingImageLink.link_size === size
                          ? 'bg-[#d4af37]/15 border-[#d4af37]/40 text-[#d4af37]'
                          : 'bg-white/5 border-white/8 text-gray-400 hover:bg-white/10'
                      }`}
                    >
                      {size === 'small' ? '小（2列）' : '大（全幅）'}
                    </button>
                  ))}
                </div>
                {/* Size preview */}
                <div className="mt-2 flex gap-1.5 h-8 items-end">
                  {editingImageLink.link_size === 'small' ? (
                    <>
                      <div className="flex-1 bg-white/10 rounded" style={{ aspectRatio: '5/7', maxHeight: '28px' }} />
                      <div className="flex-1 bg-white/10 rounded" style={{ aspectRatio: '5/7', maxHeight: '28px' }} />
                    </>
                  ) : (
              ...
              <label className="block cursor-pointer">
                <div className={`w-full rounded-xl border-2 border-dashed border-white/15 flex flex-col items-center justify-center overflow-hidden transition-colors hover:border-white/30 ${editingImageLink.previewSrc ? 'border-solid border-white/20' : ''}`}
                  style={{ aspectRatio: '5/7', maxHeight: '240px' }}>
                  {editingImageLink.previewSrc || editingImageLink.image_url ? (
                    ...
                    <div className="text-center py-6">
                      ...
                      <p className="text-[10px] text-gray-700 mt-0.5">比率 5:7</p>
                    </div>
                  )}
                </div>
              ...
              <input type="url" value={editingImageLink.url} onChange={(e) => setEditingImageLink({ ...editingImageLink, url: e.target.value })}
                ...
                className="w-full px-4 py-3 bg-white/5 border border-white/8 rounded-xl text-white placeholder-gray-700 focus:outline-none focus:border-[#d4af37]/40 text-[14px]" />
              <div className="flex gap-2">
                <button onClick={handleSaveImageLink}
          ...
          {imageLinks.length === 0 ? (
            ...
            <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleImageLinkDragEnd}>
              <SortableContext items={imageLinks.map((l) => l.id)} strategy={verticalListSortingStrategy}>
                <div className="space-y-2">
                  {imageLinks.map((link) => (
                    <SortableImageRow
                      key={link.id}
                      link={link}
                      onEdit={() => {
                        setEditingImageLink({ id: link.id, url: link.url, image_url: link.image_url, previewSrc: null, file: null, link_size: link.link_size ?? 'small' })
                        setShowImageForm(true)
                      }}
        ...
        <section className="bg-[#111] rounded-3xl border border-white/8 p-5">
          <div className="flex items-center justify-between mb-5">
            ...
            {galleryPhotos.length < 3 && (
              <button
                onClick={() => {
                  setEditingGalleryPhoto({ id: null, image_url: null, previewSrc: null, file: null })
          ...
          {showGalleryForm && editingGalleryPhoto && (
            <div className="mb-4 p-4 bg-white/5 border border-white/8 rounded-2xl space-y-3">
              <label className="block cursor-pointer">
                <div className="w-1/2 mx-auto rounded-xl border-2 border-dashed border-white/15 flex flex-col items-center justify-center overflow-hidden transition-colors hover:border-white/30"
              ...
              <div className="flex gap-2">
                <button onClick={handleSaveGalleryPhoto}
                ...
                <div key={photo.id} className="relative group">
                  ...
                  <div className="absolute inset-0 rounded-xl bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-1.5">
                    <div className="flex gap-1">
                      <button
                        onClick={() => handleGalleryMove(photo.id, 'up')}
                        disabled={idx === 0}
                      ...
                      <button
                        onClick={() => handleGalleryMove(photo.id, 'down')}
...
function SortableImageRow({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="flex items-center gap-3 px-3 py-3 rounded-2xl border border-white/8 bg-white/5"
    >
      {/* Drag handle */}
      ...
      {/* Thumbnail */}
      <div className="flex-none rounded-lg overflow-hidden bg-white/8" style={{ width: 36, aspectRatio: '5/7' }}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[10px] text-gray-600 mt-0.5">{link.link_size === 'large' ? '大（全幅）' : '小（2列）'}</p>
      </div>
