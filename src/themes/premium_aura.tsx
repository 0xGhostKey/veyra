...
// ── Conqueror's Haki — canvas jagged lightning ───────────────────────────────
function HakiCanvas() {
  const ref = useRef<HTMLCanvasElement>(null)
  useEffect(() => {
    const c = ref.current; if (!c) return
    ...
    // Midpoint displacement — jagged lightning path
    function mkPath(x1:number,y1:number,x2:number,y2:number,depth:number):[number,number][] {
      ...
      const disp=(Math.random()-.5)*len*0.80
    ...
    function buildPaths(base:number) {
      ...
      for(let b=0;b<nb;b++){
        ...
        const bl=len*(0.25+Math.random()*0.40)
    ...
    function stroke(pts:[number,number][],a:number,main:boolean){
      ...
      // black outer haze
      ctx.save();ctx.strokeStyle=`rgba(0,0,8,${a*0.60})`;ctx.lineWidth=w*11;ctx.shadowBlur=0;go();ctx.stroke();ctx.restore()
      // dark indigo glow
      ctx.save();ctx.shadowBlur=20;ctx.shadowColor='rgba(25,0,90,1)';ctx.strokeStyle=`rgba(25,0,90,${a*0.82})`;ctx.lineWidth=w*6;go();ctx.stroke();ctx.restore()
      // bright violet
      ctx.save();ctx.shadowBlur=10;ctx.shadowColor='rgba(140,0,255,1)';ctx.strokeStyle=`rgba(140,0,255,${a*0.93})`;ctx.lineWidth=w*2.8;go();ctx.stroke();ctx.restore()
      // white-hot core
      ctx.save();ctx.shadowBlur=4;ctx.shadowColor='rgba(255,210,255,1)';ctx.strokeStyle=`rgba(255,255,255,${a})`;ctx.lineWidth=w*0.9;go();ctx.stroke();ctx.restore()
    }

    type Bolt = {
      base:number; path:[number,number][]; branches:[number,number][][]
      alpha:number; target:number; decay:number; next:number
    }
    ...
    const bolts:Bolt[]=Array.from({length:12},(_,i)=>{
      ...
      return { base, path, branches, alpha:0.13, target:0.13, decay:0.92, next:Math.floor(Math.random()*100) }
    })
    ...
    const tick=()=>{
      ...
      for(const b of bolts){
        if(frame>=b.next){
          ...
          b.target=0.65+Math.random()*0.35; b.decay=0.88+Math.random()*0.09
          b.next=frame+85+Math.floor(Math.random()*140)
        }
        b.alpha+=(b.target-b.alpha)*0.14
        b.target=Math.max(0.11,b.target*b.decay)
    ...
    <>
      <style>{`
        ...
        @keyframes halo {
          ...
          33%    { opacity:.92; transform:scale(1.18); filter:hue-rotate(120deg); }
        ...
        @keyframes crtA { 0%,100%{transform:translateX(-16px) scaleX(1.0);opacity:.55} 50%{transform:translateX(16px) scaleX(1.45);opacity:.80} }
        @keyframes crtB { 0%,100%{transform:translateX( 13px) scaleX(1.0);opacity:.48} 50%{transform:translateX(-13px) scaleX(1.38);opacity:.72} }
        @keyframes crtC { 0%,100%{transform:translateX( -9px) scaleX(1.1);opacity:.52} 50%{transform:translateX( 19px) scaleX(0.82);opacity:.76} }
      ...
      <div style={{ position:'relative', minHeight:'100vh', overflowX:'hidden', zIndex:1 }}>
        ...
        {/* ── Aurora horizontal bands (6 layers) ── */}
        {[
          ...
          <div key={i} aria-hidden style={{
            ...
            background:`linear-gradient(180deg,transparent 0%,rgba(${a.col},.60) 38%,rgba(${a.col},.75) 52%,rgba(${a.col},.60) 66%,transparent 100%)`,
            filter:'blur(28px)',
            animation:`${a.anim} ${a.dur} ease-in-out infinite ${a.del}`,
        ...
        {CURTAINS.map((ct,i)=>{
          ...
          return (
            <div key={i} aria-hidden style={{
              ...
              background:`linear-gradient(180deg,transparent 0%,rgba(${ct.col},.25) 20%,rgba(${ct.col},.50) 45%,rgba(${ct.col},.25) 72%,transparent 92%)`,
              filter:'blur(24px)',
              animation:`${anims[i%3]} ${ct.dur} ease-in-out infinite ${ct.del}`,
              pointerEvents:'none', zIndex:0,
            }}/>
          )
        })}
            ...
            <div style={{ display:'flex',flexDirection:'column',alignItems:'center',marginBottom:52,animation:'profIn 1.1s cubic-bezier(.22,1,.36,1) both' }}>
              ...
              <div style={{ marginBottom:44, animation:'float 5s ease-in-out infinite' }}>
                <div style={{ position:'relative', width:148, height:148 }}>
                  ...
                  {/* Canvas Haki lightning — extends 106px outside this box */}
                  <HakiCanvas />

                  {/* Halo */}
                  <div style={{ position:'absolute',inset:-36,borderRadius:'50%',background:'conic-gradient(from 0deg,#ff0077,#aa00ff,#0055ff,#00ffcc,#ff0077)',filter:'blur(38px)',animation:'halo 3.8s ease-in-out infinite' }} />

                  {/* Ring 1 */}
                  <div style={{ position:'absolute',inset:0,borderRadius:'50%',background:'conic-gradient(from 0deg,#ff006e,#9900ff,#0055ff,#00e5cc,#aaff00,#ff9900,#ff006e)',animation:'rcw 3.8s linear infinite' }} />
                  <div style={{ position:'absolute',inset:3,borderRadius:'50%',background:'#04020e' }} />
                  {/* Ring 2 */}
                  <div style={{ position:'absolute',inset:5,borderRadius:'50%',background:'conic-gradient(from 180deg,#00ffcc,#0055ff,#9900ff,#ff006e,#00ffcc)',animation:'rccw 8.5s linear infinite' }} />
                  <div style={{ position:'absolute',inset:8,borderRadius:'50%',background:'#060116' }} />
                  {/* Ring 3 */}
                  <div style={{ position:'absolute',inset:10,borderRadius:'50%',background:'conic-gradient(from 60deg,#9900ff,#ff006e,#ff9900,#00e5cc,#0055ff,#9900ff)',animation:'rcw 6.2s linear infinite' }} />
                  <div style={{ position:'absolute',inset:13,borderRadius:'50%',background:'#060218' }} />

                  {/* Avatar */}
                  <div style={{ position:'absolute',inset:16,borderRadius:'50%',overflow:'hidden',display:'flex',alignItems:'center',justifyContent:'center',background:'linear-gradient(135deg,#0e0826,#081836)' }}>
                    {profile.avatar_url
                  ...
                  {/* Sparkles (zIndex:10 → above HakiCanvas z:5) */}
                  ...
                  <div style={{ position:'absolute',top:'12%',left:'12%',width:6,height:6,borderRadius:'50%',background:'#fff',boxShadow:'0 0 8px 3px rgba(255,140,195,.92),0 0 16px 6px rgba(255,80,155,.55)',animation:'spkD 3.7s ease-in-out infinite .25s',zIndex:10 }} />
                  <div style={{ position:'absolute',top:'12%',right:'12%',width:6,height:6,borderRadius:'50%',background:'#fff',boxShadow:'0 0 8px 3px rgba(140,165,255,.92),0 0 16px 6px rgba(100,125,255,.55)',animation:'spkD 4.1s ease-in-out infinite .78s',zIndex:10 }} />
                  <div style={{ position:'absolute',bottom:'12%',left:'12%',width:5,height:5,borderRadius:'50%',background:'#fff',boxShadow:'0 0 7px 2px rgba(180,255,210,.90),0 0 14px 5px rgba(80,255,170,.52)',animation:'spkD 3.5s ease-in-out infinite 1.2s',zIndex:10 }} />
                  <div style={{ position:'absolute',bottom:'12%',right:'12%',width:6,height:6,borderRadius:'50%',background:'#fff',boxShadow:'0 0 8px 3px rgba(255,210,140,.92),0 0 16px 6px rgba(255,170,80,.55)',animation:'spkD 3.9s ease-in-out infinite 1.8s',zIndex:10 }} />
                </div>
              </div>
