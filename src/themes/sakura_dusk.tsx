...
export default function SakuraDuskTheme({ profile, links }: Props) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center py-12 px-4" style={{ background: '#13090d' }}>
      <style>{`
        html, body { background-color: #13090d !important; }
        ...
        @keyframes sakuraGlow {
          0%, 100% { box-shadow: 0 0 8px rgba(232,124,168,0.35), 0 0 20px rgba(232,124,168,0.12); }
          50%       { box-shadow: 0 0 16px rgba(232,124,168,0.55), 0 0 32px rgba(232,124,168,0.22); }
        }
        @keyframes sakuraSep {
      ...
      <div className="w-full max-w-md" style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div className="flex justify-center gap-5 mb-6" style={{ opacity: 0.3 }}>
          <SakuraFlower size={9} color="#e87ca8" />
          <SakuraFlower size={13} color="#e87ca8" />
          <SakuraFlower size={9} color="#e87ca8" />
        </div>
        ...
        <div className="flex flex-col items-center mb-10">
          {profile.avatar_url ? (
            <div
              ...
              style={{ background: 'linear-gradient(135deg, #e87ca8, #8a2040)', animation: 'sakuraGlow 3.5s ease-in-out infinite' }}
            >
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <span style={{ display: 'inline-block', animation: 'sakuraSep 4s ease-in-out infinite' }}>
              <SakuraFlower size={14} color="#e87ca8" opacity={0.9} />
            </span>
            <div className="w-8 h-px" style={{ background: '#e87ca8' }} />
          </div>
          {profile.bio && (
        ...
        <MixedLinks
          ...
          renderTextLink={(link) => (
            <a
              ...
              style={{ background: '#1e0a12', border: '1px solid rgba(232,124,168,0.3)', color: '#e87ca8', boxShadow: '0 2px 12px rgba(232,124,168,0.08)' }}
            >
              {link.title}
            </a>
          )}
