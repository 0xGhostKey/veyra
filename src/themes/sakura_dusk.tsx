    ...
    <div className="min-h-screen flex flex-col items-center py-12 px-4" style={{ background: '#13090d' }}>
        ...
        {PETALS.map((p, i) => (
          <div key={i} style={{ position: 'absolute', top: p.top, right: '-15px', animation: `sakuraPetal ${p.dur}s ease-in-out ${p.delay}s infinite` }}>
            <svg width={p.size} height={Math.round(p.size * 0.65)} viewBox="0 0 20 13">
              <ellipse cx="10" cy="6.5" rx="10" ry="6" fill="#e87ca8" fillOpacity={p.op} />
      ...
      <div className="w-full max-w-md" style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div className="flex justify-center gap-5 mb-6" style={{ opacity: 0.3 }}>
          <span style={{ color: '#e87ca8', fontSize: 9 }}>✿</span>
          <span style={{ color: '#e87ca8', fontSize: 13 }}>✿</span>
          <span style={{ color: '#e87ca8', fontSize: 9 }}>✿</span>
        </div>
        ...
        <div className="flex flex-col items-center mb-10">
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <span style={{ color: '#e87ca8', fontSize: 15, display: 'inline-block', animation: 'sakuraSep 4s ease-in-out infinite' }}>✿</span>
            <div className="w-8 h-px" style={{ background: '#e87ca8' }} />
          </div>
          {profile.bio && (
