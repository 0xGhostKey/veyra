import type { ThemeDefinition } from '@/types'

export const THEMES: ThemeDefinition[] = [
  {
    id: 'free_basic',
    name: 'Basic',
    description: 'シンプルで清潔感のある白背景テーマ',
    price: 0,
    isFree: true,
    isAnimated: false,
    stripePriceId: null,
    previewColors: ['#ffffff', '#f3f4f6', '#111827'],
    series: 'free',
    prerequisiteId: null,
  },
  {
    id: 'free_dark',
    name: 'Simple Dark',
    description: 'スタイリッシュなダークテーマ',
    price: 0,
    isFree: true,
    isAnimated: false,
    stripePriceId: null,
    previewColors: ['#0a0a0a', '#1a1a1a', '#ffffff'],
    series: 'free',
    prerequisiteId: null,
  },
  {
    id: 'luxury_ivory',
    name: 'Luxury Ivory',
    description: 'アイボリー×アンティークゴールドの洗練されたテーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_LUXURY_IVORY ?? null,
    previewColors: ['#f8f5ef', '#a07840', '#ede8de'],
    series: 'luxury',
    prerequisiteId: null,
  },
  {
    id: 'luxury_black',
    name: 'Luxury Black',
    description: 'ゴールドアクセントで高級感を演出',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_LUXURY_BLACK ?? null,
    previewColors: ['#0a0a0a', '#d4af37', '#1a1a1a'],
    series: 'luxury',
    prerequisiteId: 'luxury_ivory',
  },
  {
    id: 'luxury_navy',
    name: 'Luxury Navy',
    description: 'ディープネイビー×ゴールドの上質なテーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_LUXURY_NAVY ?? null,
    previewColors: ['#0a0f1e', '#c8a96e', '#1a2540'],
    series: 'luxury',
    prerequisiteId: 'luxury_black',
  },

  {
    id: 'neon_violet',
    name: 'Neon Violet',
    description: 'パープル×グリーンのサイバーパンクネオンテーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_NEON_VIOLET ?? null,
    previewColors: ['#050510', '#bf00ff', '#39ff14'],
    series: 'neon',
    prerequisiteId: null,
  },
  {
    id: 'neon_glow',
    name: 'Neon Glow',
    description: 'ピンク×シアンのサイバーネオンテーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_NEON_GLOW ?? null,
    previewColors: ['#0d0d0d', '#ff00ff', '#00ffff'],
    series: 'neon',
    prerequisiteId: 'neon_violet',
  },
  {
    id: 'neon_amber',
    name: 'Neon Amber',
    description: 'オレンジ×ゴールドの熱いネオンテーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_NEON_AMBER ?? null,
    previewColors: ['#0a0800', '#ff8c00', '#ffd700'],
    series: 'neon',
    prerequisiteId: 'neon_glow',
  },

  // ── Sakura series ──
  {
    id: 'sakura_white',
    name: 'Sakura White',
    description: '薄桜×ダスティローズの和モダンなライトテーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_SAKURA_WHITE ?? null,
    previewColors: ['#fdf6f8', '#c4849a', '#3d2030'],
    series: 'sakura',
    prerequisiteId: null,
  },
  {
    id: 'sakura_dusk',
    name: 'Sakura Dusk',
    description: '夜桜の深紅×チェリーピンクのダークテーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_SAKURA_DUSK ?? null,
    previewColors: ['#13090d', '#e87ca8', '#8a2040'],
    series: 'sakura',
    prerequisiteId: 'sakura_white',
  },
  {
    id: 'sakura_matcha',
    name: 'Sakura Matcha',
    description: '抹茶の深緑×桜ピンクのコントラストテーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_SAKURA_MATCHA ?? null,
    previewColors: ['#0a100a', '#7ab87a', '#e8a4b0'],
    series: 'sakura',
    prerequisiteId: 'sakura_dusk',
  },

  // ── Chrome series ──
  {
    id: 'chrome_silver',
    name: 'Chrome Silver',
    description: 'スティールブラック×シルバーの金属質感テーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_CHROME_SILVER ?? null,
    previewColors: ['#0e0f11', '#b8bec8', '#606872'],
    series: 'chrome',
    prerequisiteId: null,
  },
  {
    id: 'chrome_gold',
    name: 'Chrome Gold',
    description: 'ディープブラック×バーニッシュゴールドの金属光沢テーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_CHROME_GOLD ?? null,
    previewColors: ['#0c0a00', '#c8a830', '#e8d060'],
    series: 'chrome',
    prerequisiteId: 'chrome_silver',
  },
  {
    id: 'chrome_platinum',
    name: 'Chrome Platinum',
    description: 'ディープブラック×プラチナホワイトの純粋な金属感',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_CHROME_PLATINUM ?? null,
    previewColors: ['#08090c', '#dce4ec', '#8090a0'],
    series: 'chrome',
    prerequisiteId: 'chrome_gold',
  },
  {
    id: 'chrome_void',
    name: 'Chrome Void',
    description: '漆黒の宇宙×エレクトリックブルーのSFメタルテーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_CHROME_VOID ?? null,
    previewColors: ['#03040a', '#4090e0', '#80b8f0'],
    series: 'chrome',
    prerequisiteId: 'chrome_platinum',
  },

  // ── Ocean series ──
  {
    id: 'ocean_abyss',
    name: 'Ocean Abyss',
    description: '深海の暗黒×バイオルミネッセントシアンの幻想テーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_OCEAN_ABYSS ?? null,
    previewColors: ['#020c14', '#00d4ff', '#003870'],
    series: 'ocean',
    prerequisiteId: null,
  },
  {
    id: 'ocean_lagoon',
    name: 'Ocean Lagoon',
    description: 'トロピカルラグーン×エメラルドターコイズのリゾートテーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_OCEAN_LAGOON ?? null,
    previewColors: ['#02100e', '#40e0b0', '#0a6050'],
    series: 'ocean',
    prerequisiteId: 'ocean_abyss',
  },
  {
    id: 'ocean_arctic',
    name: 'Ocean Arctic',
    description: '北極海の静寂×氷晶ブルーの澄んだテーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_OCEAN_ARCTIC ?? null,
    previewColors: ['#04080e', '#a0d8f8', '#2050a0'],
    series: 'ocean',
    prerequisiteId: 'ocean_lagoon',
  },

  // ── Ember series ──
  {
    id: 'ember_red',
    name: 'Ember Red',
    description: '灼熱の炎×深紅のエンバーテーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_EMBER_RED ?? null,
    previewColors: ['#0f0300', '#ff4400', '#ff7700'],
    series: 'ember',
    prerequisiteId: null,
  },
  {
    id: 'ember_ash',
    name: 'Ember Ash',
    description: '灰色の煙と灰のひんやりしたテーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_EMBER_ASH ?? null,
    previewColors: ['#111010', '#b0a090', '#d0c8bc'],
    series: 'ember',
    prerequisiteId: 'ember_red',
  },
  {
    id: 'ember_gold',
    name: 'Ember Gold',
    description: '黄金の炎×ゴールドのリッチなエンバーテーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_EMBER_GOLD ?? null,
    previewColors: ['#0c0700', '#ffaa00', '#ffd060'],
    series: 'ember',
    prerequisiteId: 'ember_ash',
  },

  // ── Botanical series ──
  {
    id: 'botanical_moss',
    name: 'Botanical Moss',
    description: '深緑の苔×葉のナチュラルダークテーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_BOTANICAL_MOSS ?? null,
    previewColors: ['#080c05', '#7ec850', '#a0e870'],
    series: 'botanical',
    prerequisiteId: null,
  },
  {
    id: 'botanical_ivory',
    name: 'Botanical Ivory',
    description: 'クリーム×アイボリーの明るいボタニカルテーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_BOTANICAL_IVORY ?? null,
    previewColors: ['#f5f0e8', '#5a8040', '#4a6830'],
    series: 'botanical',
    prerequisiteId: 'botanical_moss',
  },
  {
    id: 'botanical_earth',
    name: 'Botanical Earth',
    description: 'テラコッタ×アースカラーのウォームなボタニカルテーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_BOTANICAL_EARTH ?? null,
    previewColors: ['#1a1008', '#c87040', '#e09060'],
    series: 'botanical',
    prerequisiteId: 'botanical_ivory',
  },

  // ── Glitch series ──
  {
    id: 'glitch_green',
    name: 'Glitch Green',
    description: 'マトリックスグリーン×ターミナルの近未来グリッチテーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_GLITCH_GREEN ?? null,
    previewColors: ['#000800', '#00ff41', '#008020'],
    series: 'glitch',
    prerequisiteId: null,
  },
  {
    id: 'glitch_white',
    name: 'Glitch White',
    description: 'ホワイトノイズ×スタティックのモノクロームグリッチテーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_GLITCH_WHITE ?? null,
    previewColors: ['#0a0a0a', '#ffffff', '#606060'],
    series: 'glitch',
    prerequisiteId: 'glitch_green',
  },
  {
    id: 'glitch_red',
    name: 'Glitch Red',
    description: 'エラーレッド×警告アラートのデンジャーグリッチテーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_GLITCH_RED ?? null,
    previewColors: ['#080000', '#ff0020', '#ff4040'],
    series: 'glitch',
    prerequisiteId: 'glitch_white',
  },

  // ── Snow series ──
  {
    id: 'snow_night',
    name: 'Snow Night',
    description: '真夜中の雪景色。星空×氷晶ブルーの幻想テーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_SNOW_NIGHT ?? null,
    previewColors: ['#020615', '#a8d4ff', '#dce8ff'],
    series: 'snow',
    prerequisiteId: null,
  },
  {
    id: 'snow_aurora',
    name: 'Snow Aurora',
    description: 'オーロラ×雪の幻想的な二色テーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_SNOW_AURORA ?? null,
    previewColors: ['#08060f', '#80ffcc', '#b070ff'],
    series: 'snow',
    prerequisiteId: 'snow_night',
  },
  {
    id: 'snow_powder',
    name: 'Snow Powder',
    description: '真っ白な粉雪の昼。冬の青空をイメージしたライトテーマ',
    price: 300,
    isFree: false,
    isAnimated: true,
    stripePriceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_SNOW_POWDER ?? null,
    previewColors: ['#f0f5ff', '#3060c0', '#1a2a4a'],
    series: 'snow',
    prerequisiteId: 'snow_aurora',
  },
]

export function getThemeById(id: string): ThemeDefinition | undefined {
  return THEMES.find((t) => t.id === id)
}

export const FREE_THEME_IDS = THEMES.filter((t) => t.isFree).map((t) => t.id)
export const PAID_THEME_IDS = THEMES.filter((t) => !t.isFree).map((t) => t.id)
